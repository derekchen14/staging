...
DESIRED_DACTS = ["visualize", "explain"]
...
def annotate(data, inverted, samples):
  ...
  for idx, sample in enumerate(samples):
    ...
    exemplars = np.random.choice(options, min(6, len(options)), replace=False)
    ...
    print(f'Annotating data for <{dact}>')
    print(f'Seed example: {sample["utterance"]}')

    for utt in parsed_examples:
      print(f"Generated example: {utt}")
...
if __name__ == "__main__":
  print(" ___ Soleda Annotation ___")
  print("Initializing service ...")
  # tm.sleep(2)

  data = json.load(open('annotations/dacts_demo.json', 'r'))
  inverted = build_reverse_index(data)
  # samples = sample_dacts(data, 5)
  # samples = sample_dacts(data, 5)
  hh = input("How many new labels do you want to generate? ")
  print("  ")
  print("Sampling seed data ...")
  samples = data[-3:]
  # tm.sleep(4)
  ...
  # json.dump(data, open('annotations/dacts.json', 'w'), indent=4)
